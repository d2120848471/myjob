...
| 短信配置 | `SMSConfigService` | `internal/controller/admin/config.go` | `internal/logic/admin/config.go` | 短信配置读取、保存与脱敏展示 |
| 审计日志 | `AuditLogService` | `internal/controller/admin/log.go` | `internal/logic/admin/log.go` | 操作日志、登录日志查询 |

## 基础目录职责

| 目录 | 主要职责 | 备注 |
| --- | --- | --- |
| `api` | 请求 / 响应协议定义 | 按业务域拆分协议结构，不承载业务逻辑 |
| `internal/bootstrap` | 应用装配、路由绑定、运行入口对接 | 启动链路核心 |
| `internal/cmd` | GoFrame 命令入口 | 根应用启动命令 |
| `internal/consts` | 后台通用常量 | 状态值、通用枚举 |
| `internal/controller/admin` | HTTP 协议层 | 参数解析、响应输出 |
| `internal/service` | 模块接口边界 | 供 controller 依赖 |
| `internal/logic/admin` | 业务编排层 | 组合 kernel 与 library |
| `internal/kernel` | 运行时核心与迁移期公共数据能力 | 当前重构里的核心支撑层 |
| `internal/dao` | GoFrame DAO 模型装配 | 为进一步自动生成预留统一入口 |
| `internal/model/do` | 数据写入 / 条件对象 | 贴近持久化层 |
| `internal/model/entity` | 数据库实体结构 | 贴近表结构 |
| `internal/middleware` | 中间件 | 当前主要是鉴权与授权 |
| `internal/library` | 跨模块基础能力 | auth / sms / audit / region / response |
