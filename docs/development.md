...
- `open_order.token`：外部下单和查单接口使用的固定 token，支持 `${OPEN_ORDER_TOKEN:-default}` 形式从环境变量读取。
- `open_order.worker_enabled`：是否启动进程内订单 worker；本地默认配置为 `true`，测试默认配置为 `false`。
- `open_order.poll_interval_seconds`：订单查单轮询间隔，默认 30 秒；配置小于等于 0 时自动回退到 30。
- `open_order.submit_scan_interval_seconds`：待提交订单兜底扫描间隔，默认 5 秒；配置小于等于 0 时自动回退到 5。
