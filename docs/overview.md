...
- 外部接口 `/api/open/orders*` 支持固定 token 下单和查单
- 一期只支持直充、渠道供货商品，并只对接云发卡真实下单/查单
- 进程内 worker 负责待提交订单扫描、云发卡提交、轮询查单和窗口内智能补单
- 后台接口 `/api/admin/orders` 使用 `order.manage` 权限，支持列表筛选和今日/昨日统计
- 对外查单不暴露上游订单号、渠道、成本和利润

## 当前兼容约束

- 后台接口统一挂在 `/api/admin/*`，外部开放订单接口挂在 `/api/open/*`
