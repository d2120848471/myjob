...
// SettingsSMSSaveReq 用于保存短信发送配置。
//
// KeepAccessKey / KeepAccessKeySecret 用于“保留已有密钥”：
// - 首次配置时必须显式填写 AccessKey/Secret（不能选择保留）
// - 已配置过时可选择保留，从而避免在页面回显或重复提交敏感值
...
// SettingsSMSSaveRes 表示短信配置保存成功（返回体为空）。
